...
fn main() -> anyhow::Result<()> {
    ...
    loop {
        ...
        match msg {
            MudMessage::EnterRoom { room, other_players } => {
                ...
                if !other_players.is_empty() {
                    println!("Other players here: {}", other_players.join(", "));
                } else {
                    println!("You are alone in this room.");
                }
        ...
        if current_room.as_ref().map(|r| r.exits.iter().any(|e| e.direction.to_lowercase() == command)).unwrap_or(false) {
